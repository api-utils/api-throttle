...
public class ThrottlePolicyServiceTest {
    ...
    @Before
    public void setUp() {
        this.service = new ThrottlePolicyService();
    }

    @Test
    public void checkTest() {
        BucketStatus status = new BucketStatus("10", "0", 5L);
        BucketPolicy policy = BucketPolicyBuilder.of("bucket").nToken(10L).build();
        // check token pass
        assertTrue(this.service.check(policy, 0L, status));

        status = new BucketStatus("0", "0", 5L);
        policy = BucketPolicyBuilder.of("bucket").nToken(10L).build();
        // check token fail
        assertFalse(this.service.check(policy, 0L, status));

        status = new BucketStatus("10", "0", 5L);
        policy = BucketPolicyBuilder.of("bucket").window("60", "10").nToken("10").interval("3").build();
        // check interval fail
        ...
        policy = BucketPolicyBuilder.of("bucket").window("60", "1").nToken("10").interval("3").build();
        // check window fails
        assertFalse(this.service.check(policy, 4000L, status));

        status = new BucketStatus("10", "0", 10L);
        policy = BucketPolicyBuilder.of("bucket").window("60", "15").nToken("10").interval("3").build();
        // check window pass
        assertTrue(this.service.check(policy, 4000L, status));
    }
